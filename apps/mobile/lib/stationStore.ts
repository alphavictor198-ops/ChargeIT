...
class StationStore {
  ...
  private initializeStations() {
    this.stations = CHARGING_HUBS.map((h, i) => ({
      ...
      longitude: h.lng,
      totalSlots: 4,
      availableSlots: Math.floor(Math.random() * 3) + 1, // Start with some initial occupancy
      maxPowerKw: h.power_kw,
      pricePerKwh: 18.50 + (Math.random() * 5), // Varying pricing
      waitTimeMins: 0
    }));
  ...
  private calculateWaitTimes() {
    this.stations.forEach(s => {
      ...
      // Simple formula: each occupied slot adds some pressure to the hub
  ...
  // Simulates real-time flux from other users
